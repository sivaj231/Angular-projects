...
@Component({
  selector: 'app-download-invalid-mobile',
  templateUrl: './download-invalid-mobile.component.html',
  styleUrls: ['./download-invalid-mobile.component.css']
})
export class DownloadInvalidMobileComponent implements OnInit {
  minFromDate: Date;
  minToDate: Date;
  ...
  async download(formValues) {
    ...
    if (this.calendarStartDate.length == 0 && this.calendarEndDate.length == 0) {
      ...
      const blob = await this.fileService.downloadInvalidExcelFile(this.startDate, this.endDate);
      let fileName='InvalidMobileData.xlsx';
